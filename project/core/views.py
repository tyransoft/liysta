...
def statistics_dashboard(request):
    ...
    'month_actives_percentage': round(month_actives_percentage, 1) if month_actives_percentage is not None else 0,
    'actives_percentage': round(actives_percentage, 1) if actives_percentage is not None else 0,
    'activetion_converted_percentage': round(activetion_converted_percentage, 1) if activetion_converted_percentage is not None else 0,
    'subscriptions_count': subs_count,
    'active_subscriptions': active_subscriptions,
    'active_subs_percentage': round(active_subs_percentage, 1) if active_subs_percentage is not None else 0,
    ...
    'converted_percentage': round(converted_percentage, 1) if converted_percentage is not None else 0,
    'operations_amount': operations_amount,
    'marketing_amount': marketing_amount,
    'seller_amount': seller_amount,
    'gross_profit': gross_profit,
    'gross_percentage': round(gross_percentage, 1) if gross_percentage is not None else 0,
    'net_profit': net_profit,
    'net_percentage': round(net_percentage, 1) if net_percentage is not None else 0,
    'new_coast': new_coast,
    ...
    if request.method == 'POST':
        ...
        if delivery_type == 'darbasabil':
            ...
            if nawris_value and nawris_value != '':
              if nawris_value[0] == 'a':  
                area_id = nawris_value[1:]  
                try:
                 nawris_obj = NawrisArea.objects.get(area_id=int(area_id))
...
def manage_storage(request,menu_id):
    ...
    products_total = Products.objects.filter(menu__id=menu_id).aggregate(
